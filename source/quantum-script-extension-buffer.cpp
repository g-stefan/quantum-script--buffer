		...
		namespace Extension {
			namespace Buffer {
				...
				static void newContext(Executive *executive, void *extensionId) {
					...
					((Context::getGlobalObject())->operatorReferenceOwnProperty(bufferContext->symbolFunctionBuffer))=defaultPrototypeFunction;
				...
				static TPointer<Variable> getU8(VariableFunction *function, Variable *this_, VariableArray *arguments) {
					...
					if(!VariableBuffer::isVariableBuffer(this_)) {
				...
				static TPointer<Variable> set_(VariableFunction *function, Variable *this_, VariableArray *arguments) {
					...
					if(VariableString::isVariableString(inV)) {
					...
					if(VariableBuffer::isVariableBuffer(inV)) {
				...
				static TPointer<Variable> xorBuffer(VariableFunction *function, Variable *this_, VariableArray *arguments) {
					...
					Buffer8Core::xorOperation(&((VariableBuffer *)this_)->buffer.buffer[st], ln, ((VariableBuffer *)buffer.value())->buffer.buffer, ((VariableBuffer *)buffer.value())->buffer.length);
					return this_;
				};

				static TPointer<Variable> xorAvalancheEncode(VariableFunction *function, Variable *this_, VariableArray *arguments) {
#ifdef QUANTUM_SCRIPT_DEBUG_RUNTIME
					...
					Buffer8Core::xorAvalancheEncode(((VariableBuffer *)this_)->buffer.buffer, ((VariableBuffer *)this_)->buffer.length);
					return this_;
				};
				...
				void initExecutive(Executive *executive, void *extensionId) {
					...
					executive->setFunction2("Buffer.prototype.xor(buffer,start,ln)", xorBuffer);
...
extern "C" QUANTUM_SCRIPT_EXTENSION_BUFFER_EXPORT void quantumScriptExtension(Quantum::Script::Executive *executive, void *extensionId) {
